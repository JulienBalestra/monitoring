...
func NewMemoryReporter(conf *collecter.Config) *Memory {
	return &Memory{
		conf: conf,
		metricsMapping: map[string]string{
...
func (c *Memory) collectMetrics() (datadog.GaugeList, error) {
...
func (c *Memory) Collect(ctx context.Context) {
	ticker := time.NewTicker(c.conf.CollectInterval)
	defer ticker.Stop()
	log.Printf("collecting memory/meminfo metrics every %s", c.conf.CollectInterval.String())
	for {
		select {
		case <-ctx.Done():
			log.Printf("end of memory/meminfo collection")
			return

		case <-ticker.C:
			gauges, err := c.collectMetrics()
			if err != nil {
				log.Printf("failed memory/meminfo collection: %v", err)
				continue
			}
			gauges.Gauge(c.conf.MetricsCh)
			log.Printf("successfully run memory/meminfo collection")
		}
	}
}
